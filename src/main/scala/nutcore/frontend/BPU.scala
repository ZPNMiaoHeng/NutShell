...
class TableAddr(val idxBits: Int) extends NutCoreBundle {
  ...
  def tagBits = VAddrBits - padLen - idxBits    // in_order: 39 -3 -9 = 27
  ...
  def fromUInt(x: UInt) = x.asTypeOf(UInt(VAddrBits.W)).asTypeOf(this) // x 的位宽调整为 TableAddr 类型的位宽，并返回一个 TableAddr 类型的对象
...
class BPU_inorder extends NutCoreModule {
  val io = IO(new Bundle {
    val in = new Bundle { val pc = Flipped(Valid((UInt(VAddrBits.W)))) }    // 给pc添加一个Valid控制信号，通过Flipped反转信号接口
    val out = new RedirectIO
    val flush = Input(Bool())
  ...
  // BTB
  val NRbtb = 512
  val btbAddr = new TableAddr(log2Up(NRbtb)) // btb table：包含各种类型。idxBits=9； padLen=3; tagBits= 27
  def btbEntry() = new Bundle {
    val tag = UInt(btbAddr.tagBits.W)   // 27.W , 保存指令信息,对比
    val _type = UInt(2.W)               // BTB 保存指令类型
    val target = UInt(VAddrBits.W)      // 39.W
    val brIdx = UInt(3.W)              //FIXME - 选择BTB？？？
    val valid = Bool()
  }
  ...
  val flushBTB = WireInit(false.B)
  val flushTLB = WireInit(false.B)
  BoringUtils.addSink(flushBTB, "MOUFlushICache")    // NOTE - 使用BoringUtils将无架构分层，不同模块的信号进行链接。 MOU module  addSource 
  BoringUtils.addSink(flushTLB, "MOUFlushTLB")
  btb.reset := reset.asBool || (flushBTB || flushTLB)
  Debug(reset.asBool || (flushBTB || flushTLB), "[BPU-RESET] bpu-reset flushBTB:%d flushTLB:%d\n", flushBTB, flushTLB)

  btb.io.r.req.valid := io.in.pc.valid
  btb.io.r.req.bits.setIdx := btbAddr.getIdx(io.in.pc.bits)    // FIXME - 9.W，BTB 索引


  val btbRead = Wire(btbEntry())
  btbRead := btb.io.r.resp.data(0)    // FIXME -  ？？？？
  // since there is one cycle latency to read SyncReadMem,
  // we should latch the input pc for one cycle
  val pcLatch = RegEnable(io.in.pc.bits, io.in.pc.valid) // valid 有效后，延迟一周期得到数据
  ...
  // PHT
  val pht = Mem(NRbtb, UInt(2.W))
  val phtTaken = RegEnable(pht.read(btbAddr.getIdx(io.in.pc.bits))(1), io.in.pc.valid)  // 两比特饱和计数器

  // RAS

  val NRras = 16
  val ras = Mem(NRras, UInt(VAddrBits.W))  // 16 * 39
  ...
  when (reqLatch.valid && ALUOpType.isBranch(reqLatch.fuOpType)) {
    val taken = reqLatch.actualTaken
    val newCnt = Mux(taken, cnt + 1.U, cnt - 1.U)
    val wen = (taken && (cnt =/= "b11".U)) || (!taken && (cnt =/= "b00".U))    // 11/00 不会写入,降低功耗
    when (wen) {
      pht.write(btbAddr.getIdx(reqLatch.pc), newCnt)
      //Debug(){
  ...
  when (req.valid) {
    when (req.fuOpType === ALUOpType.call)  {  //ECall
      ras.write(sp.value + 1.U, Mux(req.isRVC, req.pc + 2.U, req.pc + 4.U))
...
class DummyPredicter extends NutCoreModule {
  val io = IO(new Bundle {
    val in = new Bundle { val pc = Flipped(Valid((UInt(VAddrBits.W)))) }
    val out = new RedirectIO      // target rtype valid
    val valid = Output(Bool())
    val flush = Input(Bool())
    val ignore = Input(Bool())
    val brIdx = Output(Vec(4, Bool()))  // FIXME - Function ???
  })
